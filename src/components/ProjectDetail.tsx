      ...
      {project && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-x-4 z-50 mx-auto max-w-[1080px] flex flex-col overflow-hidden rounded-xl border border-border bg-surface shadow-[0_25px_80px_-20px_rgba(0,0,0,0.6)]"
            style={{
              // iOS PWA + iPhone notch / home indicator: previous
              // top-[4vh] / bottom-[4vh] sat the modal under the dynamic
              // island on standalone-mode iOS. Adding the safe-area
              // insets pushes the modal cleanly inside the visible
              // viewport while keeping the 4vh editorial breathing room
              // on desktop where the insets are 0.
          ...
          >
            ...
            {/* Close button — sits on the NON-scrolling modal frame so it
                stays put while the inner content scrolls. Previously this
                was absolutely positioned inside the same element that had
                overflow-y-auto, so it scrolled away with the content the
                first time a user tried to read a long project card. */}
            <button
              onClick={onClose}
            ...
            {/* Scrolling content area — overflow lives here now so the
                close button on the frame above stays visible at all
                scroll positions. */}
